...
type View = 'chat' | 'editor' | 'seismic' | 'settings';
...
export default function App() {
  ...
  function apply(action: () => TimelineState) { try { setTimeline(action()); setError(''); } catch (e) { setError(e instanceof Error ? e.message : 'No se pudo modificar la línea de tiempo.'); } }
  ...
  async function analyzeTimeline() {
    ...
    await sendBridgeMessage(`Analiza mi timeline local. Duración total ${formatSeconds(timeline.duration)}. ${compact}. Indica cortes, huecos o solapamientos que debería revisar.`); setView('chat');
  }
  ...
  function updateSetting<K extends keyof AppSettings>(key: K, value: AppSettings[K]) { const next = saveSettings({ ...settings, [key]: value }); setSettings(next); }
  ...
  return <main className="app-shell">
    <header className="app-header">
      <div><span className="eyebrow">IAC33 · NEURAL RUNTIME</span><h1>Andrew 2.0</h1><p>Asistente · memoria · análisis · edición local</p></div>
      <span className="status-pill">{busy || seismicBusy ? 'Procesando' : 'Listo'}</span>
    </header>

    <button className="c33-card" type="button" onClick={() => setView('c33' as View)}>
      <span className="c33-mark">C33</span><span><strong>Expediente C33</strong><small>Hecho · Evidencia · Anomalía · Teoría · Veredicto</small></span><span>›</span>
    </button>
    ...
    <section className="content-panel">
      {view === 'chat' && <>
        <div className="section-head"><div><span className="eyebrow">ASISTENTE</span><h2>Andrew Chat</h2></div><span className="section-badge">bridge activo</span></div>
        <div className="messages" ref={messagesRef} role="log" aria-live="polite">
          {!messages.length && <div className="empty-state"><strong>Andrew listo</strong><span>Memoria persistente · bridge · respuestas en este panel</span></div>}
          {messages.map((m, i) => <article key={`${m.role}-${i}`} className={`message ${m.role}`}><span className="message-label">{m.role === 'user' ? 'Tú' : 'Andrew 2.0'}</span><div className="message-body">{m.text}</div></article>)}
          {busy && <div className="typing"><i /><i /><i /> Andrew está procesando…</div>}
        </div>
        <form className="composer" onSubmit={(e) => { e.preventDefault(); void sendMessage(); }}><input className="composer-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Mensaje para Andrew…" aria-label="Mensaje" /><button className="send-button" type="submit" disabled={!input.trim() || busy}>Enviar</button></form>
      </>}
