    ...
    <main className="app-page">
      <div className="app-shell">
        ...
        <section className="surface workspace-panel" role="tabpanel">
          ...
          {tab === 'metrics' && (
            <div className="tab-panel">
              <h2>Métricas</h2>
              <div className="metric-list">
                <div><span>Hechos evaluados</span><strong>{critical.facts.length}</strong></div>
                <div><span>Contradicciones</span><strong>{critical.contradictions.length}</strong></div>
          ...
          {tab === 'settings' && (
            <div className="tab-panel">
              <h2>Configuración</h2>
              <div className="metric-list">
                <div><span>Persistencia</span><strong>Local + backend</strong></div>
                <div><span>Bridge Android</span><strong>Disponible según runtime</strong></div>
                <div><span>Multimedia</span><strong>Cola asíncrona de proveedor</strong></div>
                <div><span>Autonomía</span><strong>ASSISTED</strong></div>
              </div>
        ...
        <section className="surface c33-panel">
          ...
          <div className="c33-input-row">
            <input className="text-input" value={topic} onChange={e => setTopic(e.target.value)} aria-label="Tema del expediente" />
            <button className="ui-button" type="button" onClick={() => setBrief(createC33Brief(topic))}>Crear expediente</button>
          </div>
          <h3>{brief.title}</h3>
