  ...
  return (
    <section style={{ display: 'grid', gap: 12 }} aria-label="Andrew Chat">
      <div style={{ minHeight: 420, maxHeight: '60vh', overflowY: 'auto', display: 'grid', gap: 10, padding: 8 }}>
        ...
        {chatMessages.map((item, index) => <article key={`${item.role}-${index}`} style={{ justifySelf: item.role === 'user' ? 'end' : 'start', maxWidth: '88%', padding: '12px 14px', borderRadius: 14, background: item.role === 'user' ? '#18324a' : '#121a24', overflowWrap: 'anywhere', lineHeight: 1.5 }}><div style={{ fontSize: 11, opacity: .65, marginBottom: 4 }}>{item.role === 'user' ? 'Tú' : 'Andrew 2.0'}</div>{item.attachment && <div style={{ fontSize: 12, opacity: .75, marginBottom: 6 }}>Adjunto: {item.attachment.name} · {item.attachment.type}</div>}{item.text}</article>)}
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <label style={{ minWidth: 48, minHeight: 44, display: 'grid', placeItems: 'center', borderRadius: 12, background: '#162333', cursor: 'pointer' }}>+<input type="file" accept="image/*,video/mp4,video/*" hidden onChange={event => void handleFile(event.target.files?.[0])} disabled={chatBusy} /></label>
