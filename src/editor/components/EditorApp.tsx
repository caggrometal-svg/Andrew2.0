...
export default function EditorApp():JSX.Element{
 const state=useTimelineStore(); const canvasRef=useRef<HTMLCanvasElement>(null); const rendererRef=useRef<WebGLRenderer|null>(null); const inputRef=useRef<HTMLInputElement>(null); const [ratio,setRatio]=useState<keyof typeof dimensions>("16:9"); const [exporting,setExporting]=useState(false); const [progress,setProgress]=useState(0);
 useEffect(()=>{const canvas=canvasRef.current;if(!canvas)return;const renderer=new WebGLRenderer(canvas);renderer.resize(dimensions[ratio][0],dimensions[ratio][1]);rendererRef.current=renderer;return()=>{renderer.dispose();rendererRef.current=null};},[ratio]);
 useEffect(()=>{const renderer=rendererRef.current;if(!renderer)return;const clips=state.clips.map(c=>({...c,asset:state.assetsMap[c.assetId]} as TimelineClip & {asset:unknown}));renderer.setClips(clips);renderer.setTime(state.currentTime);renderer.render();},[state.clips,state.assetsMap,state.currentTime]);
 ...
 const addText=():void=>{const track=state.tracks[0];if(!track)return;const textAssetId=crypto.randomUUID();state.addClip({...defaults(textAssetId,track.id,"Texto",5),kind:"text",text:"Nuevo texto"})};
 const exportVideo=async():Promise<void>=>{setExporting(true);setProgress(0);try{await videoExporter.export({width:dimensions[ratio][0],height:dimensions[ratio][1],fps:state.fps},{onProgress:p=>setProgress(p.percentage),onComplete:r=>videoExporter.download(r)})}finally{setExporting(false)}};
