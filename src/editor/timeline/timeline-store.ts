...
const initial = (): TimelineState => ({ tracks: [{ id: id(), name: "Video 1", order: 0, muted: false, locked: false, visible: true }], clips: [], selectedClipId: null, selectedTrackId: null, currentTime: 0, duration: 0, fps: 30, playing: false, loop: false, zoom: 1 });
...
export const useTimelineStore = create<TimelineStore>()((set, get) => ({
  ...
  selectClip: (selectedClipId) => set({ selectedClipId: selectedClipId ?? null }), selectTrack: (selectedTrackId) => set({ selectedTrackId: selectedTrackId ?? null }),
  ...
  removeAsset: (assetId) => set((s) => { const assetsMap = { ...s.assetsMap }; delete assetsMap[assetId]; const clips = s.clips.filter((c) => c.type !== "media" || c.assetId !== assetId); return { assetsMap, clips, selectedClipId: s.selectedClipId && clips.some((c) => c.id === s.selectedClipId) ? s.selectedClipId : null }; }),
