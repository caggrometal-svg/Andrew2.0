...
interface DragState { clipId: UUID; pointerId: number; startX: number; startTime: number; trackId: UUID; startTrackIndex: number }
...
export function useTimelineDrag(options: Options): { onPointerDown: (event: ReactPointerEvent<HTMLElement>, clipId: UUID) => void } {
  ...
  const onMove = useCallback((event: PointerEvent): void => {
    ...
    const tracks = [...store.tracks].sort((a, b) => a.order - b.order); const trackDelta = Math.round((event.clientY - (d.startY ?? event.clientY)) / Math.max(1, options.trackHeight ?? 52)); const target = tracks[d.startTrackIndex + trackDelta]; store.moveClip(d.clipId, Math.max(0, startTime), target?.id ?? d.trackId);
