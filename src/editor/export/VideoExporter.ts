...
import type { Muxer as MuxerType } from "mp4-muxer";
...
type EncoderMetadata = Parameters<MuxerType["addVideoChunk"]>[1];
...
export class VideoExporter {
  ...
  async export(options: VideoExportOptions = {}, callbacks: VideoExportCallbacks = {}): Promise<VideoExportResult> {
    ...
    try {
      ...
      encoder = new Encoder({ output: (chunk, metadata) => { if (muxer) muxer.addVideoChunk(chunk as Parameters<MuxerType["addVideoChunk"]>[0], metadata); }, error: (error) => { encoderError = error instanceof Error ? error : new Error(String(error)); } });
      ...
      for (let frameIndex = 0; frameIndex < totalFrames; frameIndex += 1) {
        if (this.cancelled) throw abortError(); const timestamp = Math.round((frameIndex / fps) * 1_000_000); const time = Math.min(duration, frameIndex / fps); renderer.setClips(clips); renderer.setTime(time);
        ...
        if (encoderError) throw encoderError; renderer.render(); const frame = new Frame(canvas, { timestamp, duration: frameDuration }); try { encoder.encode(frame, { keyFrame: frameIndex === 0 || frameIndex % Math.max(1, fps * 2) === 0 }); } finally { frame.close(); } callbacks.onProgress?.(progress(frameIndex, totalFrames, time, duration));
      }
      if (this.cancelled) throw abortError(); callbacks.onStatusChange?.("finalizing"); await encoder.flush(); if (encoderError) throw encoderError; muxer.finalize(); const blob = new Blob([target.buffer.slice(0)], { type: "video/mp4" }); const result: VideoExportResult = { blob, mimeType: "video/mp4", filename, duration, width: size.width, height: size.height, fps, frames: totalFrames }; callbacks.onProgress?.(progress(totalFrames, totalFrames, duration, duration)); callbacks.onStatusChange?.("completed"); callbacks.onComplete?.(result); return result;
    } catch (error) { if (error instanceof DOMException && error.name === "AbortError") { callbacks.onStatusChange?.("cancelled"); throw error; } const normalized = error instanceof Error ? error : new Error(String(error)); callbacks.onStatusChange?.("error"); callbacks.onError?.(normalized); throw normalized; }
    finally { try { encoder?.close(); } catch { } renderer?.dispose(); this.exporting = false; }
  }
