...
function createSafeId(prefix: string): string {
  try { const random = globalThis.crypto?.randomUUID?.(); if (random) return `${prefix}${random}`; } catch { }
  try { const bytes = new Uint8Array(16); globalThis.crypto?.getRandomValues?.(bytes); const entropy = Array.from(bytes, byte => byte.toString(16).padStart(2, '0')).join(''); if (entropy) return `${prefix}${Date.now().toString(36)}-${entropy}`; } catch { }
  return `${prefix}${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`;
}

function getBackendUrl(): string { return ANDREW_BACKEND_URL; }
...
function getAndrewUserId(): string {
  try {
    ...
    const generated = createSafeId('user:'); window.localStorage.setItem(USER_ID_STORAGE_KEY, generated); return generated;
  } catch { return createSafeId('session:'); }
}

function readStoredMemory(): string[] {
  try { const raw = window.localStorage.getItem(MEMORY_STORAGE_KEY); if (!raw) return []; const parsed = JSON.parse(raw) as unknown; if (!Array.isArray(parsed)) return []; return parsed.filter((value): value is string => typeof value === 'string' && value.trim().length > 0).slice(-20); } catch { return []; }
}
function writeStoredMemory(values: string[]): void { try { window.localStorage.setItem(MEMORY_STORAGE_KEY, JSON.stringify(values.slice(-50))); } catch { } }
...
function extractExplicitMemory(message: string): string[] {
  ...
  if (nameMatch?.[1]) { const name = nameMatch[1].trim().replace(/\s+/g, ' '); if (name) { try { window.localStorage.setItem(USER_NAME_STORAGE_KEY, name); } catch { } facts.push(`El nombre del usuario es ${name}.`); } }
...
function buildLocalMemory(currentMessage: string, supplied: AndrewMemoryContext[] | undefined): string[] {
  ...
  const identity = userName ? [`El nombre del usuario es ${userName}.`] : []; let chat: string[] = [];
...
function notify(listener: NetworkStatusListener | undefined, status: NetworkStatus, detail?: string): void { listener?.(status, detail); try { window.dispatchEvent(new CustomEvent('andrew:network-status', { detail: { status, detail } })); } catch { } }
...
async function fetchWithRetry(input: NetworkRequestInput, init: RequestInit, timeoutMs: number, listener?: NetworkStatusListener, attempts = MAX_RETRIES): Promise<Response> {
  ...
  for (let attempt = 0; attempt <= attempts; attempt += 1) {
    ...
    try {
      ...
      if (!retryableStatus || attempt === attempts) return response; await sleep(BACKOFF_MS * (2 ** attempt));
    } catch (error) {
      lastError = error;
...
function parseError(data: unknown, fallback: string): Error {
  if (data && typeof data === 'object') {
    const value = data as AndrewChatError;
    if (typeof value.message === 'string' && value.message.trim()) { const failed = value.failures?.map(item => `${item.provider}: ${item.reason}`).join(' | '); return new Error(failed ? `${value.message} ${failed}` : value.message); }
    if (typeof value.error === 'string' && value.error.trim()) return new Error(value.error);
  }
  return new Error(fallback);
}
function haptic(duration = 12): void { try { if ('vibrate' in navigator) navigator.vibrate(duration); } catch { } }
...
async function uploadChunk(base: string, uploadId: string, fileSize: number, start: number, end: number, chunk: ArrayBuffer, listener?: NetworkStatusListener): Promise<number> {
  ...
  for (let attempt = 0; attempt < VIDEO_CHUNK_RETRIES; attempt += 1) {
    try {
      ...
      if (!response.ok || !data.ok || data.received !== end) throw parseError(data, `Falló el fragmento ${start}-${end}.`); return data.received;
