...
describe('IAC33 functional kernel', () => {
  ...
  it('runs prediction -> outcome -> Brier -> learning -> next prediction', () => {
    ...
    const prediction = kernel.createPrediction({
      domain: 'general',
      hypothesis: 'The primary scenario will occur.',
      signals: [{ name: 'signal', value: 0.8, weight: 1 }],
      evidence: [{ id: 'src-1', kind: 'evidence', source: 'public-source' }],
    });
    ...
    const resolved = kernel.resolvePrediction(prediction.id, true, 'Observed outcome matched the primary scenario.');
    ...
    const next = kernel.createPrediction({
      domain: 'general',
      hypothesis: 'Second forecast uses historical performance.',
      signals: [{ name: 'signal', value: 0.8, weight: 1 }],
    });
  ...
  it('isolates learning by project and domain', () => {
    ...
    const prediction = a.createPrediction({
      domain: 'earthquake',
      hypothesis: 'Test earthquake scenario.',
      signals: [{ name: 'signal', value: 0.7 }],
    });
    a.resolvePrediction(prediction.id, false, 'The event did not occur.');
    ...
    expect(validateLearningMemory('project-a', 'earthquake').valid).toBe(true);
  });

  it('supports permission, activity, memory update and deletion', () => {
    const kernel = new Iac33Kernel('project-a');
    kernel.setPermission('memory.write', 'allow', 'test');
    expect(kernel.getPermissions()[0].decision).toBe('allow');

    const prediction = kernel.createPrediction({
      domain: 'social',
      hypothesis: 'Test social scenario.',
      signals: [{ name: 'signal', value: 0.6 }],
    });
