...
export function createHealthCheck(name: string, status: HealthStatus, latencyMs?: number, detail?: string): HealthCheck {
  ...
  const base: HealthCheck = Object.freeze({ name: normalizedName, status });
  if (normalizedLatency !== undefined && detail !== undefined) return Object.freeze({ ...base, latencyMs: normalizedLatency, detail: detail.trim() });
  if (normalizedLatency !== undefined) return Object.freeze({ ...base, latencyMs: normalizedLatency });
  if (detail !== undefined) return Object.freeze({ ...base, detail: detail.trim() });
  return base;
}
...
export function createStructuredHealth(input: {
  ...
  return Object.freeze({ status: normalizeStatus(checks), version: input.version.trim(), uptimeMs: Math.round(input.uptimeMs), timestamp: input.timestamp ?? new Date().toISOString(), checks });
}
