...
const isTestRuntime = process.env.NODE_ENV === 'test' || process.env.PHASE25_E2E === '1';
const rateLimitMax = isTestRuntime ? 1000 : config.rateLimitMax;

export async function buildServer({ listen = false, port = config.port, host = config.host } = {}) {
  ...
  await app.register(cors, {
    ...
    allowedHeaders: ['Accept', 'Authorization', 'Content-Type', 'Origin', 'X-Requested-With', 'X-Andrew-User-Id', 'X-Andrew-Signature', 'X-Andrew-Timestamp', 'X-Andrew-Nonce', 'X-Andrew-Permissions', 'X-Chunk-Start', 'X-Chunk-End', 'X-Upload-Size'],
  ...
  await app.register(rateLimit, {
    ...
    max: rateLimitMax,
    timeWindow: '1 minute',
    errorResponseBuilder: (_request, context) => ({ ok: false, error: 'RATE_LIMITED', message: `Demasiadas solicitudes. Intenta nuevamente en ${Math.ceil(context.ttl / 1000)} segundos.` }),
  });

  const chatRateLimit = app.createRateLimit({ max: rateLimitMax, timeWindow: '1 minute' });
  const telemetryRateLimit = app.createRateLimit({ max: isTestRuntime ? 1000 : 60, timeWindow: '1 minute' });
  const gatewayRateLimit = app.createRateLimit({ max: isTestRuntime ? 1000 : 30, timeWindow: '1 minute' });
  ...
  app.addHook('onRequest', async (request, reply) => {
    ...
    const limiter = request.url === '/api/chat'
      ...
      : request.url === '/api/telemetry'
        ? telemetryRateLimit
        : (request.url.startsWith('/api/v1/sessions') || request.url.startsWith('/api/v1/tools') || request.url.startsWith('/api/v1/bridge/v3'))
          ? gatewayRateLimit
          : undefined;
    if (!limiter) return;
  ...
  const authError = (error) => ({
    bridge_auth_required: 'MISSING_SECURITY_HEADERS',
    bridge_auth_expired: 'TIMESTAMP_EXPIRED',
    bridge_auth_replay: 'REPLAY_ATTACK_DETECTED',
    bridge_identity_unregistered: 'UNREGISTERED_DEVICE_IDENTITY',
    bridge_auth_invalid: 'INVALID_SIGNATURE',
  ...
  app.addHook('preValidation', async (request, reply) => {
    ...
    if (!result.ok) return reply.code(401).send({ ok: false, error: authError(result.error) });
  ...
  app.addHook('onSend', async (request, reply, payload) => {
    ...
    const userId = resolveUserId(request);
    if (!userId || !shouldLearn(message)) return payload;
    try {
      ...
      request.log.error({ error }, 'persistent learning write failed');
    }
    return payload;
  });
  ...
  app.get('/health', async () => {
    ...
    const learningConfigured = memoryConfigured;
    const sessionConfigured = memoryConfigured;
    const ai = getAIProviderHealth();
    const aiProviders = Object.values(ai.providers);
    const aiConfigured = aiProviders.some((provider) => provider.configured);
    const aiOperational = aiProviders.some((provider) => provider.configured && provider.score > 0 && provider.state !== 'open' && provider.lastErrorClass !== 'permanent');
    const aiDegraded = aiConfigured && !aiOperational;
    ...
    const healthy = openaiConfigured && aiConfigured && aiOperational && memoryConfigured && ffmpeg.available && ffmpeg.executable && ffprobe.available && ffprobe.executable;
    return {
      ok: healthy,
      status: healthy ? 'healthy' : 'degraded',
      service: 'andrew2-backend',
      latencyMs: Number(latencyMs.toFixed(3)),
      checks: { server: 'ok', openaiApiKeyConfigured: openaiConfigured, aiProvidersConfigured: aiConfigured, aiProviderOperational: aiOperational, aiProviderDegraded: aiDegraded, memoryStoreConfigured: memoryConfigured, learningStoreConfigured: learningConfigured, sessionStoreConfigured: sessionConfigured, mediaRuntime: ffmpeg.available && ffmpeg.executable && ffprobe.available && ffprobe.executable ? 'ok' : 'degraded' },
      ai,
      environment: { port, host, corsOriginsConfigured: config.corsOrigins.length },
      system,
      metrics: snapshotMetrics(),
      media: { video: 'chunked-temp', generation: 'openai-videos', ffmpeg, ffprobe },
    };
  });
  ...
  if (listen) await app.listen({ port, host });
  return app;
}

const entrypoint = process.argv[1] ? fileURLToPath(import.meta.url) === fileURLToPath(`file://${process.argv[1]}`) : false;
if (entrypoint) await buildServer({ listen: true });
