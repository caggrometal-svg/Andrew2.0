...
export async function registerVideoRoutes(app) {
  ...
  app.put('/api/media/video/:uploadId', {
    ...
    try {
      await handle.write(chunk, 0, chunk.length, start);
    } finally {
      await handle.close();
    }
    ...
    const complete = upload.received === upload.size;
    if (complete) {
      const finalPath = upload.filePath;
      return reply.send({ ok: true, uploadId: upload.id, received: upload.received, complete: true, temporaryPath: finalPath, mimeType: upload.mimeType, name: upload.name });
    }
    return reply.send({ ok: true, uploadId: upload.id, received: upload.received, complete: false });
  });
