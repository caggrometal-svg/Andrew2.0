...
export async function enqueueBridgeCommand({ userId, envelope }) {
  ...
  const db = getPool();
  await db.query(`INSERT INTO andrew_bridge_commands (id,user_id,command,payload,created_at,expires_at) VALUES ($1,$2,$3,$4::jsonb,to_timestamp($5/1000.0),to_timestamp($6/1000.0))`, [envelope.id, cleanId, envelope.command, envelope.payload === undefined ? null : JSON.stringify(envelope.payload), envelope.createdAt, envelope.expiresAt]);
  await db.query(`DELETE FROM andrew_bridge_commands WHERE user_id=$1 AND (expires_at<=NOW() OR id IN (SELECT id FROM andrew_bridge_commands WHERE user_id=$1 AND acknowledged_at IS NULL ORDER BY created_at DESC OFFSET $2))`, [cleanId, MAX_PENDING]);
  return envelope;
}

async function deleteExpired(userId) { await getPool().query('DELETE FROM andrew_bridge_commands WHERE user_id=$1 AND expires_at<=NOW()', [userId]); }
...
export async function getBridgeSyncState(userId) {
  const pending = await listPendingBridgeCommands(userId);
...
export async function acknowledgeBridgeCommand({ userId, id, ok, error, result = null }) {
  ...
  const updated = await getPool().query(`UPDATE andrew_bridge_commands SET acknowledged_at=NOW(),ack_ok=$3,ack_error=$4,result=$5::jsonb WHERE id=$1 AND user_id=$2 AND acknowledged_at IS NULL AND expires_at>NOW() RETURNING id,command`, [id, cleanId, ok, error || null, resultJson]);
