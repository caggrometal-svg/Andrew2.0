...
export async function registerControlPlaneRoutes(app) {
  app.get('/api/v1/control/status', async (request, reply) => {
    ...
    return { ok: true, service: 'andrew-control-plane', protocol: 'gpt-control-v1', commands: [...COMMANDS], ttlMs: TTL_MS };
  });
  ...
  app.post('/api/v1/control/commands', async (request, reply) => {
    ...
    if (!DEVICE_ID.test(targetDeviceId) || !COMMANDS.has(command)) return reply.code(400).send({ ok: false, error: 'invalid_control_command' });
    ...
    return reply.code(202).send({ ok: true, requestId: commandId, commandId, targetDeviceId, command, expiresAt: result.command.expiresAt, statusUrl: `/api/v1/control/commands/${commandId}` });
  });
